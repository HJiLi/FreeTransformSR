...
class ImageRestorationModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        self.accum_iter = int(
            self.opt.get('datasets', {}).get('train', {}).get(
                'accum_iter',
                train_opt.get('accum_iter', 1)
            )
        )
    ...
    def optimize_parameters(self, current_iter, tb_logger=None, accum_iter=1, update_params=True):
        # Only clear grads at the start of an accumulation window.
        ...
        # Important:
        # losses here use reduction='mean', so divide by accum_iter to keep
        # accumulated gradient equivalent to a larger batch.
        (l_total / self.accum_iter).backward()

        self._accum_counter += 1

        if self._accum_counter >= self.accum_iter:
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr=True, use_image=True):
        ...
        # 删掉了分布式的rank/world_size，单卡直接跑
        pbar = tqdm(total=len(dataloader), unit='image')

        cnt = 0

        for idx, val_data in enumerate(dataloader):
            # 删掉了分布式的分片，单卡直接遍历所有验证数据
            img_name = osp.splitext(osp.basename(val_data['lq_path'][0]))[0]
        ...
        # 删掉了分布式的reduce，单卡直接平均结果
        if with_metrics:
            metrics_dict = {}
            cnt = float(cnt)
            for key, metric in self.metric_results.items():
                metrics_dict[key] = metric / cnt
